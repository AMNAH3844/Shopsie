...
// import { View, Text, TextInput, TouchableOpacity, Alert, ScrollView, ActivityIndicator, StyleSheet, Dimensions, Platform, StatusBar } from "react-native";
...
export default function ShopDetails() {
  ...
  // const API_PROFILE_URL = "http://172.20.140.250:5000/api/my-shop-profile";

  useEffect(() => {
    fetchSavedShopDetails();
  }, []);

  // Safe window message listening across the web platform build target
  useEffect(() => {
  ...
  // Update Map Position dynamically without tearing down the native engine
  useEffect(() => {
    if (loading) return;
...
// 💡 FIXED: Encapsulated fully back inside a valid async function block
  const fetchSavedShopDetails = async () => {
  try {
    console.log("🔄 Fetching shop details initiated...");
  ...
  const fetchCurrentGPSLocation = async (quiet = false) => {
    try {
      ...
      if (status !== "granted") {
        if (!quiet) Alert.alert("Permission denied", "Allow location access to find your shop.");
        return;
      }
      ...
      if (!quiet) Alert.alert("Success", "Location updated via GPS!");
    } catch (err) {
      if (!quiet) Alert.alert("Error", "Could not detect location automatically.");
    } finally {
      setLocating(false);
    }
  };
  ...
  `);
    ...
    Alert.alert("Success", "Location found and map updated.");
  } catch (error) {
    console.error(error);
  ...
  const handleMapMessage = (event) => {
    try {
      ...
      console.error("Error reading map transmission:", err);
    }
  };
  ...
  if (!shopName || !city || !latitude || !longitude || !phone || !timing || !description) {
    Alert.alert("Error", "Please complete all registration fields.");
    return;
  }
  ...
  try {
    ...
    // Make sure this matches your NEW server route path!
...
console.log("URL:", API_URLS.SHOP_PROFILE_UPDATE);
...
console.log("SERVER RESPONSE:", data);
  ...
  setDescription(data.shop.description);
    ...
    // 💡 CRITICAL FIX: Print the exact issue to your console so we can see it
    console.error("Network Error Details:", err);
    Alert.alert("Network Error", `Could not connect to server: ${err.message}`);
  } finally {
    // This safely runs NO MATTER WHAT, stopping the infinite loading wheel
    setSaving(false);
  }
};
    ...
    <body>
      ...
      <script>
        ...
        marker.on('dragend', function(event) {
          ...
          broadcastCoords(position.lat, position.lng);
        });
...
<View style={localStyles.coordinatesGrid}>
  ...
  <View style={localStyles.coordinateHalf}>
    ...
    >
        ...
        {longitude}
          ...
          {isEditMode ? (
            <View style={localStyles.actionRow}>
              {hasExistingProfile ? (
                <>
                  <TouchableOpacity onPress={() => setIsEditMode(false)} style={[localStyles.actionButtonHalf, localStyles.cancelButtonColor]} activeOpacity={0.7}>
                    <Text style={localStyles.actionButtonText}>Cancel</Text>
                  </TouchableOpacity>
                  <TouchableOpacity onPress={handleSaveOrUpdate} style={[localStyles.actionButtonHalf, localStyles.saveButtonColor]} disabled={saving} activeOpacity={0.8}>
                    {saving ? <ActivityIndicator color="white" /> : <Text style={localStyles.actionButtonText}>Save Changes</Text>}
                  </TouchableOpacity>
                </>
              ) : (
                <TouchableOpacity onPress={handleSaveOrUpdate} style={[localStyles.fullEditButton, localStyles.saveButtonColor]} disabled={saving} activeOpacity={0.8}>
                  {saving ? <ActivityIndicator color="white" /> : <Text style={localStyles.fullEditButtonText}>Save Shop Details</Text>}
                </TouchableOpacity>
              )}
            </View>
          ) : (
            <TouchableOpacity onPress={() => setIsEditMode(true)} style={localStyles.fullEditButton} activeOpacity={0.8}>
      ...
      {/* UPDATED SHOPKEEPER NAVIGATION (HOME, STOCK, ORDERS) */}
...
const localStyles = StyleSheet.create({
 ...
 mapHeaderRow: {
  ...
  mapCardWrapper: { height: 240, width: '100%', borderRadius: 16, overflow: 'hidden', borderWidth: 1.5, borderColor: '#CBD5E1', shadowColor: '#475569', shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.14, shadowRadius: 16, elevation: 5, backgroundColor: '#FFFFFF' },
  
  coordinatesGrid: { flexDirection: 'row', justifyContent: 'space-between', marginTop: 14, marginBottom: 10 },
  coordinateHalf: { width: '48%' },
  coordLabel: { fontSize: 12, color: '#475569', fontWeight: '700', marginBottom: 6, textTransform: 'uppercase', letterSpacing: 0.3 },
  coordValueBox: { 
    flexDirection: 'row',
    alignItems: 'center',
    justifyContent: 'center',
  ...
  actionButtonHalf: { flex: 1, maxWidth: '48%', borderRadius: 14, height: 52, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', elevation: 3 },
