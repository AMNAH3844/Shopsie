  ...
  const fetchOrders = useCallback(async () => {
    try {
      ...
      triggerWarningNotification(
        error.message || "Warning: Could not fetch orders",
      );
    } finally {
      setLoading(false);
    }
      ...
      <View style={localStyles.card}>
        ...
        <View style={localStyles.metaBox}>
          ...
          <Text style={localStyles.metaText}>
            Shop: {item.shopName || "Your shop"}
          </Text>
          {item.riderName ? (
            <Text style={localStyles.metaText}>Rider: {item.riderName}</Text>
          ) : null}
          {item.product ? (
            <Text style={localStyles.metaText}>
        ...
        {isPending ? (
          <View style={localStyles.actionRow}>
            <TouchableOpacity
              ...
              onPress={() =>
                openActionConfirmation(item.id, "approve", item.productName)
              }
            ...
            <TouchableOpacity
              ...
              onPress={() =>
                openActionConfirmation(item.id, "reject", item.productName)
              }
              disabled={savingId === item.id}
              activeOpacity={0.7}
            >
      ...
      <View style={localStyles.mainContainer}>
        ...
        {loading ? (
          ...
          <FlatList
            ...
            ListHeaderComponent={
              <Text style={localStyles.sectionHeading}>Purchase Approvals</Text>
            }
            ListEmptyComponent={
              <Text style={localStyles.emptyText}>No rider purchases yet.</Text>
            }
        ...
        >
          <View style={localStyles.modalOverlay}>
            <View style={localStyles.modalBox}>
              ...
              <Text style={[localStyles.modalTitle, { color: "#2e4466" }]}>
                {pendingAction.type === "approve"
                  ? "Approve Purchase"
                  : "Reject Purchase"}
              </Text>
              <Text style={localStyles.modalSubtitle}>
                Are you sure you want to {pendingAction.type} the incoming stock
                transaction for "{pendingAction.productName}"?
              </Text>
