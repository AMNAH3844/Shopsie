...
const emptyLocation = (label) => ({ lat: DEFAULT_LAT, lng: DEFAULT_LNG, label });
...
const calculateDistanceKm = (fromLat, fromLng, toLat, toLng) => {
  if ([fromLat, fromLng, toLat, toLng].some((value) => value == null || Number.isNaN(Number(value)))) {
    return 0;
  }
...
const parseShopTiming = (timing) => {
  ...
  if (!raw || raw === "No timings set") return { label: raw || "Shop timing not provided", isOpen: null };
  ...
  const start = toMinutes(timeMatch[1], timeMatch[2], timeMatch[3], timeMatch[6]);
...
export default function Cart() {
  ...
  const [selectedSummaryModalVisible, setSelectedSummaryModalVisible] = useState(false);
  ...
  const activeLocation = locationMode === "delivery" ? deliveryLocation : selectedLocation;
  const activeLocationChosen =
    locationMode === "delivery" ? deliveryLocationChosen : locationChosen;
  const activeLocationTitle =
    locationMode === "delivery" ? "Choose Delivery Location" : "Choose Location To Buy Items From";
  ...
  const handleAnyMapMessage = async (payload) => {
    if (payload?.type === "SHOP_SELECT" && payload.shopId) {
      const shop = nearbyShops.find((s) => Number(s.shopId) === Number(payload.shopId));
    ...
    if (payload?.lat && payload?.lng) {
      ...
      const fallbackLabel =
        locationMode === "delivery" ? "Selected delivery location" : "Selected buying location";
    ...
    itemsWithShops.forEach((item) => {
      item.options?.forEach((opt) => {
        ...
        const alreadyAdded = shop.items.some(
          (existing) => existing.itemKey === item.itemKey && existing.productId === opt.productId
        );
  ...
  const loadRegisteredShopsAndOpenMap = async () => {
    ...
    try {
      ...
      if (shops.length === 0) {
        triggerWarning(`No matching shops found within ${SEARCH_RADIUS_KM} km.`);
      } else {
        const autoCount = Object.keys(autoSelected).length;
        triggerWarning(`Auto-selected best shops for ${autoCount} of ${cartItems.length} item(s).`);
      }
    } catch (e) {
      console.log("Cart shop load error:", e.response?.data || e.message);
      Alert.alert("Error", "Could not load registered shops for selected area.");
    } finally {
  ...
  const createAssignedItem = (shop, shopItem) => ({
    ...
    deliveryLocationLabel: deliveryLocationChosen ? deliveryLocation.label : null,
    lineTotal:
      Number(shopItem.price || 0) *
      Math.min(Number(shopItem.requestedQuantity || 1), Number(shopItem.availableQuantity || 0)),
  });
  ...
  const buildAutoSelectedItems = (shops) => {
    ...
    shops.forEach((shop) => {
      ...
      shop.items.forEach((shopItem) => {
        ...
        if (!candidatesByItem.has(shopItem.itemKey)) candidatesByItem.set(shopItem.itemKey, []);
    ...
    candidatesByItem.forEach((candidates, itemKey) => {
      const [best] = candidates.sort((a, b) => {
        const priceDiff = Number(a.shopItem.price || 0) - Number(b.shopItem.price || 0);
        if (priceDiff !== 0) return priceDiff;

        const coverageDiff =
          Number(shopCoverage.get(b.shop.shopId) || 0) - Number(shopCoverage.get(a.shop.shopId) || 0);
        if (coverageDiff !== 0) return coverageDiff;

        return Number(a.shop.distanceKm || 0) - Number(b.shop.distanceKm || 0);
      });

      if (best) selected[itemKey] = createAssignedItem(best.shop, best.shopItem);
    });
  ...
  const selectedCartItems = useMemo(() => {
    return cartItems.map((item) => {
      ...
      const baseLocation = {
        ...
        buyingLocationLng: selectedLocation.lng,
        buyingLocationLabel: selectedLocation.label,
        deliveryLocationLat: deliveryLocationChosen ? deliveryLocation.lat : assigned?.deliveryLocationLat || null,
        deliveryLocationLng: deliveryLocationChosen ? deliveryLocation.lng : assigned?.deliveryLocationLng || null,
        deliveryLocationLabel: deliveryLocationChosen ? deliveryLocation.label : assigned?.deliveryLocationLabel || null,
      };
  ...
  }, [cartItems, selectedItemsByKey, selectedLocation, deliveryLocation, deliveryLocationChosen]);
  ...
  const categoryFilters = useMemo(() => {
    const categories = new Set(priceComparison.map((item) => item.categoryName).filter(Boolean));
    return ["ALL", ...Array.from(categories)];
  }, [priceComparison]);
  const filteredPriceComparison = useMemo(() => {
    if (activeCategoryFilter === "ALL") return priceComparison;
    return priceComparison.filter((item) => item.categoryName === activeCategoryFilter);
  }, [activeCategoryFilter, priceComparison]);

  const total = selectedCartItems.reduce((sum, item) => sum + Number(item.lineTotal || 0), 0);
  const selectedUniqueShops = useMemo(() => {
    const shopMap = new Map();
    selectedCartItems.forEach((item) => {
      if (!item.selectedShopId || shopMap.has(item.selectedShopId)) return;
      const sourceShop = nearbyShops.find((shop) => shop.shopId === item.selectedShopId);
  ...
  const shopsDistanceKm = selectedUniqueShops.reduce((sum, shop) => sum + Number(shop.distanceKm || 0), 0);
  const deliveryDistanceKm = deliveryLocationChosen
    ? calculateDistanceKm(selectedLocation.lat, selectedLocation.lng, deliveryLocation.lat, deliveryLocation.lng)
    : 0;
  ...
  const selectedCount = selectedCartItems.filter((item) => item.selectedShopId).length;
  const pendingCount = Math.max(cartItems.length - selectedCount, 0);

  const handleFinalize = () => {
    if (!locationChosen) return triggerWarning("Choose a buying location first.");
    if (nearbyShops.length === 0) return triggerWarning("Open registered shops map first.");
  ...
  const handleShareInitiation = (type) => {
    if (!locationChosen || !deliveryLocationChosen) {
      return triggerWarning("First choose both buying location and delivery location.");
    }
    ...
    if (missing.length > 0) {
      return triggerWarning(`Remove or select shops for ${missing.length} item(s) before sharing.`);
    }
...
const proceedShare = () => {
  ...
  if (selectedShareType === "Friend") {
    router.push({ pathname: "/customerDashboard/friends", params: commonParams });
    return;
  }
...
document.addEventListener("message", function(e) {
  ...
  const renderLocationCard = ({ title, chosen, location, icon, onPress }) => (
    <View style={styles.locationCard}>
      ...
      <View style={styles.locationBox}>
        <Ionicons name={chosen ? "location" : "location-outline"} size={20} color="#2e4466" />
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={styles.locationText} numberOfLines={2}>{location.label}</Text>
          <Text style={styles.coordsText}>{formatCoords(location.lat, location.lng)}</Text>
        </View>
      </View>
  ...
  return (
    <View style={styles.mainContainer}>
      <LinearGradient colors={["#eef4fe", "#2e4466"]} start={{ x: 1, y: 0 }} end={{ x: 0, y: 0 }} style={styles.header}>
        ...
        <Text style={styles.headerTitleText} numberOfLines={1}>{listName}</Text>
        <View style={{ width: 28 }} />
      </LinearGradient>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.sectionCard}>
          ...
          {locationChosen && (
            <TouchableOpacity style={styles.secondaryBtn} onPress={loadRegisteredShopsAndOpenMap} disabled={shopsLoading}>
              {shopsLoading ? (
                <ActivityIndicator color="#2e4466" />
              ) : (
                <>
                  <Ionicons name="storefront-outline" size={18} color="#2e4466" />
                  <Text style={styles.secondaryBtnText}>Select Registered Shops Within {SEARCH_RADIUS_KM} km</Text>
                </>
              )}
            </TouchableOpacity>
          )}
        ...
        {!finalized && (
          <View style={styles.sectionCard}>
            <View style={styles.sectionHeaderRow}>
              ...
              <Text style={styles.countBadge}>{selectedCount}/{cartItems.length}</Text>
            </View>
    ...
    <Text style={styles.itemDetailText}>
      Spec: {item.specification || "None"}
                ...
                <View style={styles.finalActionColumn}>
                  <TouchableOpacity style={styles.removePlainBtn} onPress={() => removeCartItem(item.id)} hitSlop={8}>
            ...
            <View style={styles.billSummaryCard}>
              ...
              <View style={styles.billLine}>
                <Text style={styles.billLabel}>Distance used for charges</Text>
                <Text style={styles.billValue}>{payableDistanceKm.toFixed(2)} km</Text>
              </View>
              <View style={styles.billLine}>
                <Text style={styles.billLabel}>Delivery charges ({DELIVERY_RATE_PER_KM} Rs/km)</Text>
  ...
  {!isRequestRiderFlow && (
    ...
    >
      ...
      <Text style={styles.shareFriendText}>Share with Friend</Text>
    </TouchableOpacity>
  )}
...
>
        ...
        >
            ...
            <View style={styles.searchRow}>
              <TextInput
                ...
                placeholder={locationMode === "delivery" ? "Search delivery location" : "Search location, e.g. DHA Phase 6"}
            ...
            <View style={styles.mapBox}>
              {Platform.OS === "web" ? (
                <iframe id="cart-location-map" srcDoc={locationMapHtml} style={{ width: "100%", height: "100%", border: "none" }} title="Cart Location Map" />
              ) : (
                <WebView
  ...
  <Text style={styles.primaryBtnText}>
    {activeLocationChosen ? "Update Location" : "Done"}
          ...
          <View style={styles.shopMapModalBox}>
            <View style={styles.modalHeaderRow}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.metaText}>Showing shops within {SEARCH_RADIUS_KM} km of selected buying area</Text>
              </View>
              <TouchableOpacity style={styles.selectionSummaryBtn} onPress={() => setSelectedSummaryModalVisible(true)}>
                <Ionicons name="receipt-outline" size={16} color="#2e4466" />
                <Text style={styles.selectionSummaryText}>{selectedCount}/{cartItems.length}</Text>
              </TouchableOpacity>
              <TouchableOpacity onPress={() => setShopsMapModalVisible(false)}>
                <Ionicons name="close-circle" size={28} color="#94a3b8" />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.shopMapBox}>
              {Platform.OS === "web" ? (
                <iframe id="cart-shops-map" srcDoc={shopsMapHtml} style={{ width: "100%", height: "100%", border: "none" }} title="Registered Shops Map" />
              ) : (
                <WebView
                  ref={shopsWebViewRef}
                  originWhitelist={["*"]}
            ...
            <ScrollView style={styles.shopSideBox} showsVerticalScrollIndicator={false}>
              {nearbyShops.length === 0 ? (
                <Text style={styles.emptyText}>No matching shops found in this area.</Text>
              ) : (
                <>
                  <Text style={styles.comparisonHeading}>Compare & Pick Your Shop</Text>
                  <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.categoryFilterRow}>
                    {categoryFilters.map((category) => {
                      ...
                      return (
                        <TouchableOpacity
                          key={category}
                          style={[styles.categoryFilterPill, isActive && styles.activeCategoryFilterPill]}
                          onPress={() => setActiveCategoryFilter(category)}
                        >
                          <Text style={[styles.categoryFilterText, isActive && styles.activeCategoryFilterText]}>
                  ...
                  {filteredPriceComparison.map((item) => (
                    <View key={item.itemKey} style={styles.comparisonBlock}>
                      <View style={styles.comparisonItemHeader}>
                        <View style={{ flex: 1 }}>
                          <Text style={styles.comparisonItemName}>Item: {item.itemName}</Text>
                          <Text style={styles.metaText}>Spec: {item.specification} | Qty: {item.requestedQuantity}</Text>
                        </View>
                        <Text style={styles.categoryPill}>{item.categoryName}</Text>
                      </View>

                      {item.options.map((option, index) => {
                        const alreadySelected = selectedItemsByKey[item.itemKey]?.selectedShopId === option.shopId;

                        return (
                          <TouchableOpacity
                            key={`${item.itemKey}-${option.shopId}-${index}`}
                            style={[styles.comparisonOption, alreadySelected && styles.activeComparisonOption]}
                            onPress={() => assignItemToShop(option)}
                          >
                            <View style={styles.radioCircle}>{alreadySelected && <View style={styles.radioDot} />}</View>
                            ...
                            <View style={styles.optionActionColumn}>
                              ...
                              >
                                <Text style={styles.shopDetailsText}>Shop Details</Text>
                              </TouchableOpacity>
                              <Text style={styles.comparisonPrice}>Rs. {option.price}</Text>
                            </View>
                          </TouchableOpacity>
                        );
                      })}
      ...
      <Modal visible={selectedSummaryModalVisible} transparent animationType="fade" onRequestClose={() => setSelectedSummaryModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBoxLarge}>
            <View style={styles.modalHeaderRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.modalTitle}>Selected Shops Summary</Text>
                <Text style={styles.metaText}>Selected {selectedCount} of {cartItems.length} item(s)</Text>
              </View>
              <TouchableOpacity onPress={() => setSelectedSummaryModalVisible(false)}>
                <Ionicons name="close-circle" size={28} color="#94a3b8" />
              </TouchableOpacity>
            </View>

            <ScrollView style={styles.summaryListBox} showsVerticalScrollIndicator={false}>
              {selectedCartItems.map((item, index) => (
                <View key={item.id || index} style={[styles.summaryItemRow, item.selectedShopId && styles.summaryItemSelected]}>
                  <View style={styles.summaryItemHeader}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.summaryItemName}>{item.name}</Text>
                      <Text style={styles.summaryMeta}>Qty: {item.quantity} | Spec: {item.specification}</Text>
                    </View>
                    <Text style={[styles.summaryStatusPill, item.selectedShopId ? styles.summarySelectedPill : styles.summaryPendingPill]}>
            ...
            <TouchableOpacity style={styles.primaryBtn} onPress={() => setSelectedSummaryModalVisible(false)}>
      ...
      <Modal visible={shopItemsModalVisible} transparent animationType="fade" onRequestClose={() => setShopItemsModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBoxLarge}>
            <View style={styles.modalHeaderRow}>
              <View style={{ flex: 1 }}>
                <Text style={styles.modalTitle}>{selectedShop?.shopName}</Text>
                <Text style={styles.metaText}>Lat/Lng: {selectedShop?.latitude}, {selectedShop?.longitude}</Text>
                <Text style={styles.metaText}>{selectedShop?.distanceKm} km from selected buying area</Text>
                <Text
                  style={[
                    styles.metaText,
                    parseShopTiming(selectedShop?.shopTiming).isOpen === true && styles.openShopText,
                    parseShopTiming(selectedShop?.shopTiming).isOpen === false && styles.closedShopText,
                  ]}
                >
                  {getShopStatusText(selectedShop?.shopTiming)}
                </Text>
                {!!selectedShop?.shopPhone && <Text style={styles.metaText}>Phone: {selectedShop.shopPhone}</Text>}
                {!!selectedShop?.shopDescription && <Text style={styles.metaText}>{selectedShop.shopDescription}</Text>}
              </View>
              <TouchableOpacity onPress={() => setShopItemsModalVisible(false)}>
                <Ionicons name="close-circle" size={28} color="#94a3b8" />
              </TouchableOpacity>
            </View>
            ...
            <ScrollView style={{ width: "100%", maxHeight: 420 }} showsVerticalScrollIndicator={false}>
              {selectedShop?.items?.map((shopItem, index) => {
                const alreadySelected = selectedItemsByKey[shopItem.itemKey]?.selectedShopId === selectedShop.shopId;

                return (
                  <TouchableOpacity key={`${shopItem.itemKey}-${shopItem.productId}-${index}`} style={[styles.shopItemChoice, alreadySelected && styles.activeShopItemChoice]} onPress={() => assignItemToShop(shopItem)}>
                    <View style={{ flex: 1 }}>
                      <Text style={styles.itemName}>{shopItem.itemName}</Text>
                      <Text style={styles.metaText}>Category: {shopItem.categoryName} | Requested: {shopItem.specification} | Shop Spec: {shopItem.specs}</Text>
                      <Text style={styles.metaText}>Price: Rs. {shopItem.price} | Available: {shopItem.availableQuantity} of {shopItem.requestedQuantity}</Text>
                    </View>
                    <View style={styles.choiceStatus}>
                      {alreadySelected ? <Ionicons name="checkmark-circle" size={24} color="#10b981" /> : <Ionicons name="add-circle-outline" size={24} color="#2e4466" />}
                    </View>
                  </TouchableOpacity>
                );
              })}
            ...
            <TouchableOpacity style={styles.secondaryBtn} onPress={() => setShopItemsModalVisible(false)}>
              <Text style={styles.secondaryBtnText}>Done Selecting From This Shop</Text>
            </TouchableOpacity>
          </View>
        </View>
      </Modal>

      <Modal visible={shopDetailsModalVisible} transparent animationType="fade" onRequestClose={() => setShopDetailsModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <TouchableOpacity onPress={() => setShopDetailsModalVisible(false)} style={styles.closeCornerBtn}>
              <Text style={styles.closeX}>x</Text>
            </TouchableOpacity>

            <Text style={styles.modalTitle}>{detailsShop?.shopName || "Shop Details"}</Text>
            <View style={styles.detailsList}>
              <View style={styles.detailRow}>
                <Ionicons name="navigate-outline" size={20} color="#2e4466" />
                <View style={styles.detailTextBox}>
                  <Text style={styles.detailLabel}>Distance</Text>
                  <Text style={styles.detailValue}>{Number(detailsShop?.distanceKm || 0).toFixed(2)} km away</Text>
                </View>
              </View>
              <View style={styles.detailRow}>
                ...
                <View style={styles.detailTextBox}>
                  ...
                  <Text
                    style={[
                      styles.detailValue,
                      parseShopTiming(detailsShop?.shopTiming).isOpen === true && styles.openShopText,
                      parseShopTiming(detailsShop?.shopTiming).isOpen === false && styles.closedShopText,
                    ]}
                  >
              ...
              <View style={styles.detailRow}>
                ...
                <View style={styles.detailTextBox}>
                  ...
                  <Text style={styles.detailValue}>{detailsShop?.shopPhone || "Not provided"}</Text>
                </View>
              </View>
              <View style={styles.detailRow}>
              ...
              {!!detailsShop?.shopDescription && (
                <View style={styles.detailRow}>
                  <Ionicons name="document-text-outline" size={20} color="#2e4466" />
                  <View style={styles.detailTextBox}>
                    <Text style={styles.detailLabel}>About</Text>
                    <Text style={styles.detailValue}>{detailsShop.shopDescription}</Text>
                  </View>
                </View>
              )}
            </View>

            <TouchableOpacity style={styles.confirmBtnFull} onPress={() => setShopDetailsModalVisible(false)}>
      ...
      <Modal visible={itemDetailsModalVisible} transparent animationType="fade" onRequestClose={() => setItemDetailsModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            <TouchableOpacity onPress={() => setItemDetailsModalVisible(false)} style={styles.closeCornerBtn}>
              <Text style={styles.closeX}>x</Text>
            </TouchableOpacity>

            <Text style={styles.modalTitle}>{detailsItem?.name || "Item Details"}</Text>
            <View style={styles.detailsList}>
              <View style={styles.detailRow}>
                ...
                <View style={styles.detailTextBox}>
                  ...
                  <Text style={styles.detailValue}>
                    {detailsItem?.categoryName} | Qty: {detailsItem?.quantity} | Spec: {detailsItem?.specification}
                  </Text>
                </View>
              </View>

              <View style={styles.detailRow}>
                <Ionicons name="storefront-outline" size={20} color="#2e4466" />
                <View style={styles.detailTextBox}>
                  ...
                  {!!detailsItem?.selectedShopId && (
                    <>
                      <Text style={styles.detailValue}>{detailsItem.selectedShopStatusText}</Text>
                      <Text style={styles.detailSubValue}>
                        Available: {detailsItem.availableQuantity} of {detailsItem.quantity} | Total: Rs. {detailsItem.lineTotal}
                      </Text>
                    </>
                  )}
                </View>
              ...
              {!!detailsItem?.selectedShopId && (
                <View style={styles.detailRow}>
                  ...
                  <View style={styles.detailTextBox}>
                    ...
                    <Text style={styles.detailValue}>
                      {detailsItem.selectedShopLatitude}, {detailsItem.selectedShopLongitude}
                    </Text>
                  </View>
                </View>
              )}
              ...
              <View style={styles.detailRow}>
                ...
                <View style={styles.detailTextBox}>
                  ...
                  <Text style={styles.detailValue}>{detailsItem?.buyingLocationLabel || selectedLocation.label}</Text>
                  <Text style={styles.detailSubValue}>
                    {formatCoords(detailsItem?.buyingLocationLat || selectedLocation.lat, detailsItem?.buyingLocationLng || selectedLocation.lng)}
                  </Text>
                </View>
              </View>
              ...
              {deliveryLocationChosen && (
                <View style={styles.detailRow}>
                  <Ionicons name="bicycle-outline" size={20} color="#2e4466" />
                  <View style={styles.detailTextBox}>
                    <Text style={styles.detailLabel}>DELIVERY LOCATION</Text>
                    <Text style={styles.detailValue}>{deliveryLocation.label}</Text>
                    <Text style={styles.detailSubValue}>{formatCoords(deliveryLocation.lat, deliveryLocation.lng)}</Text>
                  </View>
                </View>
              )}
            </View>

            <TouchableOpacity style={styles.confirmBtnFull} onPress={() => setItemDetailsModalVisible(false)}>
              <Text style={styles.confirmBtnText}>Done</Text>
            </TouchableOpacity>
          </View>
        </View>
      ...
      <Modal visible={shareModalVisible} transparent animationType="fade" onRequestClose={() => setShareModalVisible(false)}>
        <View style={styles.modalOverlay}>
          <View style={styles.modalBox}>
            ...
            <Text style={styles.modalSubtitle}>Share {listName} with selected shops, buying location, and delivery location to a {selectedShareType}?</Text>
            <View style={styles.modalButtonsRow}>
              <TouchableOpacity style={styles.cancelBtn} onPress={() => setShareModalVisible(false)}>
                <Text style={styles.cancelBtnText}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={styles.confirmBtn} onPress={proceedShare}>
