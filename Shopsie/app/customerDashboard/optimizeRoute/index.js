...
export default function OptimizeRoute() {
  ...
  const fetchItemChoices = async () => {
    try {
      ...
      Alert.alert(
        "Error",
        "Could not load item specifications configurations.",
      );
      setLoading(false);
    }
  };
  ...
  const calculateClientSideRoute = () => {
      ...
      if (targetShops.length <= 7) {
        ...
        allPossibleSequences.forEach((sequence) => {
          ...
          for (let i = 0; i < sequence.length; i++) {
            currentSequenceDistance += getDist(
              currentLat,
              currentLng,
              sequence[i].latitude,
    ...
    const computedWaypoints = [
      { label: "Your Location", lat: startLat, lng: startLng },
    ];
    ...
    bestSequence.forEach((nextStop) => {
      ...
      computedWaypoints.push({
        label: `Stop ${letter}: ${nextStop.shopName}`,
        lat: nextStop.latitude,
        lng: nextStop.longitude,
      });

      const namesOnly = nextStop.items.map(
        (item) =>
  ...
  const filteredItems = useMemo(() => {
    return (
      rawPayload?.itemsWithShops.filter((item) => {
    ...
    return (
      ...
      >
        ...
        <Text style={{ marginTop: 12, color: "#64748b", fontWeight: "500" }}>
          Calculating shortest path permutation...
        </Text>
      </View>
    );
  }
      ...
      <View style={styles.container}>
        ...
        >
          <View style={styles.mapCard}>
            {Platform.OS === "web" ? (
              <iframe
                id="leaflet-route-iframe"
                srcDoc={mapHtmlTemplate}
                style={{ width: "100%", height: "100%", border: "none" }}
                title="OSRM Map Track Preview"
              />
            ) : (
              <WebView
                ref={webViewRef}
          ...
          <View style={styles.sectionCard}>
            ...
            >
              {categories.map((cat, idx) => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.chipButton,
                    selectedCategory === cat && styles.activeChipButton,
                  ]}
            ...
            {filteredItems.length === 0 ? (
              <Text style={styles.emptyText}>
                No products found under this category filter.
              </Text>
            ) : (
              filteredItems.map((item, idx) => (
                <View key={item.itemKey || idx} style={styles.productBlock}>
                  ...
                  >
                    <View style={{ flex: 1, paddingRight: 8 }}>
                      ...
                      <Text style={styles.specsText}>
                        Required Qty: {item.requestedQuantity} | Spec:{" "}
                  ...
                  {item.options.length === 0 ? (
                    <Text style={styles.specsText}>
                      No shops have this exact item specification in stock right
                      now.
                    </Text>
                  ) : (
                    item.options.map((opt, oIdx) => {
                      const isSelected =
                        selectedShops[item.itemKey] === opt.shopId;
                      return (
                        <TouchableOpacity
                          ...
                          style={[
                            styles.optionRow,
                            isSelected && styles.activeSelectionBorder,
                          ]}
                          onPress={() =>
                            handleSelectShop(item.itemKey, opt.shopId)
                          }
                        >
                          ...
                          <View style={{ flex: 1, marginLeft: 10 }}>
                            ...
                            <Text style={styles.specsText}>
                              Specs: {opt.specs || "None"}
                            </Text>
                            <Text style={styles.specsText}>
                              Available: {opt.availableQuantity} of{" "}
                              {opt.requestedQuantity}
                            </Text>
                          </View>
          ...
          <View style={styles.sectionCard}>
            <Text style={styles.sectionTitle}>
              Stops Sequence Based on Choice
            </Text>
            {instructions.map((item, index) => (
              <View key={index} style={styles.instructionRow}>
                <View style={styles.iconContainer}>
                  <MaterialCommunityIcons
                    name={item.icon || "directions"}
                    size={22}
                    color="#64748b"
                  />
                </View>
          ...
          <View style={styles.sectionCard}>
            <Text style={styles.sectionTitle}>
              Shortest Sequence Haul breakdown
            </Text>
            {shopGroupedSummary.length === 0 ? (
              <Text style={styles.emptyText}>
                No shop selections confirmed.
              </Text>
            ) : (
              shopGroupedSummary.map((group, index) => {
                const shopSubtotal = group.items.reduce(
                  (acc, curr) =>
                    acc + Number(curr.lineTotal || curr.price || 0),
                  0,
                );
                ...
                return (
                  <View key={index} style={styles.groupedShopContainer}>
                    <View style={styles.groupedShopHeader}>
                      <Text style={styles.groupedShopName}>
                        Stop {index + 1}: {group.shopName}
                      </Text>
                      <Text style={styles.groupedShopTotal}>
                        Total: Rs. {shopSubtotal}
                      </Text>
                    </View>
                      ...
                      {group.items.map((prod, pIdx) => (
                        <View key={pIdx} style={styles.groupedItemRow}>
                          <Text style={styles.groupedItemBullet}>
                            • {prod.name} | Spec: {prod.specification || "None"}{" "}
                            | Qty: {prod.availableQuantity} of{" "}
                            {prod.requestedQuantity}
                          </Text>
                          <Text style={styles.groupedItemPrice}>
                            Rs. {prod.lineTotal}
                          </Text>
                        </View>
                      ))}
            ...
            <View style={styles.modalContentCard}>
              <View style={styles.modalHeaderRow}>
                <Text style={styles.modalHeading}>
                  {activeShopDetails?.shopName || "Shop Details"}
                </Text>
              ...
              <ScrollView style={{ maxHeight: 300, marginVertical: 12 }}>
                <View style={styles.infoBlock}>
                  ...
                  <Text style={styles.infoValue}>
                    {activeShopDetails?.shopTiming || "No timings set"}
                  </Text>
                </View>
                ...
                <View style={styles.infoBlock}>
                  ...
                  <Text style={styles.infoValue}>
                    {activeShopDetails?.shopPhone ||
                      "No contact line available"}
                  </Text>
                </View>
                ...
                <View style={styles.infoBlock}>
                  ...
                  <Text style={styles.infoValue}>
                    {activeShopDetails?.shopDescription ||
                      "No registration bio info saved."}
                  </Text>
                </View>
              </ScrollView>
