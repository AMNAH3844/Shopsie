...
const InboxScreen = () => {
  ...
  const fetchAll = useCallback(async (isBackground = false) => {
    try {
      ...
      const riderPromise = axios
        .get(`${API_URLS.RIDER}/customer-requests`, {
          headers: { Authorization: `Bearer ${token}` },
        })
        .catch((err) => {
  ...
  const confirmDeleteRiderRequest = async () => {
    ...
    try {
      ...
      await axios.delete(`${API_URLS.RIDER}/requests/${requestId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });

      setRiderRequests((prev) => prev.filter((r) => r.id !== requestId));
      ...
      <View style={{ flex: 1 }}>
        ...
        <View style={styles.msgRow}>
          ...
          <Text
            style={[
              styles.lastMsgText,
              item.unreadCount > 0 && styles.unreadTextBold,
            ]}
            numberOfLines={1}
          >
            {item.lastMsg}
      ...
      ? [
          {
            ...
            data: filteredRiderRequests,
            type: "rider",
          },
        ]
      : []),
    ...(filteredChats.length > 0
      ? [{ title: "Messages", data: filteredChats, type: "chat" }]
      : []),
  ];
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      <View style={styles.container}>
        ...
        >
          ...
          <TouchableOpacity
            onPress={() => router.push("/customerDashboard/downloadlists")}
          >
                ...
                <>
                  <View
                    style={[
                      ...
                      {
                        backgroundColor:
                          STATUS_STYLES[detailRequest.status]?.bg || "#fef3c7",
                        borderColor:
                          STATUS_STYLES[detailRequest.status]?.border ||
                          "#fde68a",
                  ...
                  >
                    <Ionicons
                      name={
                        STATUS_STYLES[detailRequest.status]?.icon ||
                        "time-outline"
                      }
                      size={13}
                      color={
                        STATUS_STYLES[detailRequest.status]?.text || "#d97706"
                      }
                    />
                    <Text
                      style={[
                        ...
                        {
                          color:
                            STATUS_STYLES[detailRequest.status]?.text ||
                            "#d97706",
                        },
                      ]}
                    >
                  ...
                  <Text style={styles.detailMeta}>
                    Rider: {detailRequest.rider?.user?.username || "Rider"}
                  </Text>
                  {!!detailRequest.buyingLocationLabel && (
                    <Text style={styles.detailLocation}>
                      Buy from: {detailRequest.buyingLocationLabel}
                    </Text>
                  )}
                  {!!detailRequest.deliveryLocationLabel && (
                    <Text style={styles.detailLocation}>
                      Deliver to: {detailRequest.deliveryLocationLabel}
                    </Text>
                  )}
                      ...
                      <View key={idx} style={styles.detailItemRow}>
                        <View style={{ flex: 1 }}>
                          ...
                          {!!item.selectedShopName && (
                            <Text style={styles.detailShopText}>
                  ...
                  {(() => {
                    const total = (detailRequest.items || []).reduce(
        ...
        <Modal visible={!!deleteNotice} transparent animationType="fade">
          <View style={styles.overlay}>
            <View style={styles.popup}>
              ...
              <TouchableOpacity
                style={[styles.cancelBtn, { width: "100%", marginRight: 0 }]}
                onPress={() => setDeleteNotice(null)}
              >
        ...
        <Modal visible={showDeletePopup} transparent animationType="fade">
          <View style={styles.overlay}>
            <View style={styles.popup}>
              ...
              <Text style={[styles.popupTitle, { color: "#ef4444" }]}>
                Remove Request
              </Text>
              <Text style={styles.modalSubtitle}>
                Are you sure you want to permanently delete the conversation for
                "{selectedRequestForDelete?.listName}"?
              </Text>
