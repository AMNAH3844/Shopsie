...
const FriendsScreen = () => {
  ...
  const [friendToShareWith, setFriendToShareWith] = useState({
    id: null,
    name: "",
  });
  ...
  const [toastMessage, setToastMessage] = useState(
    "Please select a list to share first",
  );
  ...
  const performSearch = async () => {
    ...
    try {
      ...
      const res = await axios.get(
        `${API_URL}/search?query=${cleanQuery}&currentUserId=${currentUserId}`,
      );
    ...
    <View style={styles.friendCard}>
      <Image
        source={{
          uri:
            item.profileImage ||
            `https://ui-avatars.com/api/?name=${item.username}&background=c2ccdb&color=2e4466`,
        }}
        style={styles.profilePic}
      />
      <View style={styles.infoContainer}>
        <Text style={styles.nameText}>{item.username}</Text>
        {isSearchItem && item.friendshipStatus === "PENDING" && (
          <Text style={styles.pendingSubText}>Pending Request</Text>
        )}
        {isSearchItem && item.friendshipStatus === "ACCEPTED" && (
          <Text style={styles.statusText}>Already Friend</Text>
        )}
        {!isSearchItem && item.status === "PENDING" && !item.amIReceiver && (
      ...
      {isSearchItem ? (
        <View>
          {!item.friendshipStatus ? (
            ...
            <Ionicons
              name={
                item.friendshipStatus === "ACCEPTED" ? "people" : "time-outline"
              }
              size={22}
              color="#999"
            />
        ...
        <View style={styles.actionButtons}>
          {item.status === "PENDING" && item.amIReceiver ? (
            ...
            <View style={styles.row}>
              ...
              <TouchableOpacity
                onPress={() => confirmRemove(item.friendshipId, item.username)}
              >
    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: "#fff" }}>
      <View style={styles.container}>
        ...
        >
          ...
          <TouchableOpacity onPress={() => setShowAddSection(!showAddSection)}>
            <MaterialCommunityIcons
              name={showAddSection ? "close-circle" : "account-plus"}
              size={32}
              color="#2e4466"
            />
          </TouchableOpacity>
        ...
        {showAddSection && (
          <View style={styles.addSectionContainer}>
            ...
            <FlatList
              ...
              renderItem={({ item }) =>
                renderUserItem({ item, isSearchItem: true })
              }
              style={{ maxHeight: 250 }}
            />
          </View>
        ...
        <FlatList
          ...
          renderItem={({ item }) =>
            renderUserItem({ item, isSearchItem: false })
          }
          ...
          <View style={styles.modalOverlay}>
            <View style={styles.modalBox}>
              ...
              <Text style={styles.modalSubtitle}>
                Are you sure you want to remove "{selectedFriend.name}" from
                your friend list?
              </Text>
        ...
        >
          <View style={styles.modalOverlay}>
            <View style={styles.modalBox}>
              ...
              <Text style={styles.modalSubtitle}>
                Do you want to share your active shopping list with "
                {friendToShareWith.name}"?
              </Text>
