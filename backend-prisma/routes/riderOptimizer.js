...
const getItemDone = (item) =>
  Boolean(item.riderOptimizerDone || item.customerOptimizerDone);
const getItemDoneAt = (item) =>
  item.riderOptimizerDoneAt || item.customerOptimizerDoneAt || null;
...
const groupItemsByShop = async (items) => {
  ...
  const shopNames = Array.from(
    new Set(items.map((item) => item.selectedShopName).filter(Boolean)),
  );
  ...
  items.forEach((item) => {
    const hasExplicitShopId =
      item.selectedShopId !== null &&
      item.selectedShopId !== undefined &&
      item.selectedShopId !== "";
    ...
    const shopName =
      item.selectedShopName || details?.shopName || fallbackShop?.shopName;
    ...
    shopMap.get(key).items.push({
      ...
      lineTotal:
        item.lineTotal ||
        Number(item.selectedShopPrice || 0) * Number(item.quantity || 1),
...
const findMatchingProduct = async (item, shopkeeperId) => {
  ...
  const itemSpec = String(item.specification || "")
    .trim()
    .toLowerCase();
  return (
    candidates.find(
...
const createShopkeeperOrderNotification = async ({
  request,
  item,
  riderUserId,
}) => {
  ...
  const rider = await prisma.user.findUnique({
    where: { id: Number(riderUserId) },
  });
  const shopName =
    item.selectedShopName ||
    shopkeeper.shopDetails?.shopName ||
    shopkeeper.shopName;
  const productLabel = item.specification
...
const createCustomerShopkeeperOrderNotification = async ({
  list,
  item,
  customerUserId,
}) => {
...
router.get("/requests/:id/plan", verifyToken, async (req, res) => {
  try {
    ...
    return res
      .status(500)
      .json({ message: "Could not load rider optimizer plan" });
  }
});
...
router.post("/requests/:id/optimize", verifyToken, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(originLat) || !Number.isFinite(originLng)) {
      return res
        .status(400)
        .json({ message: "Valid rider live location is required" });
    }
    ...
    if (!access.isRider)
      return res
        .status(403)
        .json({ message: "Only rider can optimize this route" });

    const stops = await groupItemsByShop(request.items);
    const { sequence, totalDistanceKm } = optimizeStops(
    ...
    return res.json({
      ...
      estimatedMinutes:
        Math.round((totalDistanceKm / 35) * 60) + sequence.length * 5,
      stops: sequence.map((stop, index) => ({
        ...stop,
        stopNumber: index + 1,
      })),
    });
  ...
  async (req, res) => {
    try {
      ...
      if (!access.isRider)
        return res
          .status(403)
          .json({ message: "Only rider can update item status" });
...
router.post("/shops/:shopkeeperId/report", verifyToken, async (req, res) => {
  try {
    ...
    const { title, reason, source, riderRequestId, downloadedListId } =
      req.body;
    const normalizedTitle =
      title === "OTHER_REASON" ? "OTHER_REASON" : "SHOP_DOES_NOT_EXIST";
    ...
    if (countWords(cleanReason) > 500) {
      return res
        .status(400)
        .json({ message: "Reason must be 500 words or less" });
    }
    ...
    const report = await prisma.shopReport.create({
      data: {
        ...
        riderRequestId: riderRequestId ? Number(riderRequestId) : null,

        downloadedListId: downloadedListId ? Number(downloadedListId) : null,

        title: normalizedTitle,
        reason: normalizedTitle === "OTHER_REASON" ? cleanReason : null,
...
router.post("/requests/:id/delivery-route", verifyToken, async (req, res) => {
  try {
    ...
    if (!access.isRider)
      return res
        .status(403)
        .json({ message: "Only rider can create delivery route" });

    const allDone =
      request.items.length > 0 &&
      request.items.every((item) => item.riderOptimizerDone);
    if (!allDone)
      return res.status(400).json({ message: "Mark all items done first" });
...
router.get("/downloaded-lists/:id/plan", verifyToken, async (req, res) => {
  try {
    ...
    if (!list)
      return res.status(404).json({ message: "Downloaded list not found" });
    ...
    return res
      .status(500)
      .json({ message: "Could not load downloaded-list optimizer plan" });
  }
});
...
router.post("/downloaded-lists/:id/optimize", verifyToken, async (req, res) => {
  try {
    ...
    if (!Number.isFinite(originLat) || !Number.isFinite(originLng)) {
      return res
        .status(400)
        .json({ message: "Valid live location is required" });
    }
    ...
    return res
      .status(500)
      .json({ message: "Could not optimize downloaded list route" });
  }
});

router.patch(
  "/downloaded-lists/:id/items/:itemId/done",
  verifyToken,
  async (req, res) => {
    try {
      const { done } = req.body;
...
router.post(
  "/downloaded-lists/:id/delivery-route",
  verifyToken,
  async (req, res) => {
    try {
      const { riderLat, riderLng } = req.body;
      ...
      const allDone =
        list.items.length > 0 &&
        list.items.every((item) => item.customerOptimizerDone);
      if (!allDone)
        return res.status(400).json({ message: "Mark all items done first" });
      ...
      if (!Number.isFinite(deliveryLat) || !Number.isFinite(deliveryLng)) {
        return res
          .status(400)
          .json({ message: "Delivery location is missing" });
      }
      ...
      return res
        .status(500)
        .json({ message: "Could not create delivery route" });
    }
  },
);
